...
# Full fresh write-once rerun after the positive-edge TSV precision-contract
# fix.  No scientific artifact from failed attempt_002 is reused.
...
readonly OUT_ROOT="${BASE_ROOT}/04_stage3b_cellbit_negative_exact_attempt_003"
readonly CONTROL_ROOT="${BASE_ROOT}/05_stage3b_cellbit_negative_exact_control_attempt_003"
...
for required in "${MANIFEST}" "${QUALITY}" "${MARKERS}" "${TRACTOR}" \
                ...
                "${MAKEBLASTDB}" "${BLASTN}" \
                "${BASE_ROOT}/05_stage3b_cellbit_negative_exact_control_attempt_002/RUN_RESULT.json"; do
...
readonly CODE_AUTHORITY="${CONTROL_ROOT}/CODE_AUTHORITY.tsv"
...

{
  ...
  for path in \
    "${CODE_ROOT}/src/stage3b.cpp" \
    "${CODE_ROOT}/python/stage3b_signed_leiden.py" \
    "${CODE_ROOT}/tools/run_lake_stage3b_cellbit_negative_precision_attempt003.sh" \
    "${TRACTOR}" "${ANI_ENGINE}" "${SUBASS}"; do
    printf '%s\t%s\t%s\t%s\n' "$(basename "${path}")" \
      "$(stat -c '%s' "${path}")" "$(sha256sum "${path}" | awk '{print $1}')" "${path}"
  done
  prior="${BASE_ROOT}/05_stage3b_cellbit_negative_exact_control_attempt_002/RUN_RESULT.json"
  printf 'prior_failed_RUN_RESULT.json\t%s\t%s\t%s\n' "$(stat -c '%s' "${prior}")" \
    "$(sha256sum "${prior}" | awk '{print $1}')" "${prior}"
} > "${CODE_AUTHORITY}"
...
printf 'Starting fresh precision-contract Cellbit-negative Stage3B at %s\n' "$(date --iso-8601=seconds)"
printf 'threads=%s output=%s prior_scientific_artifact_reuse=false\n' "${THREADS}" "${OUT_ROOT}"
...
printf 'Finished fresh precision-contract Stage3B rc=%s elapsed=%ss at %s\n' "${rc}" "${elapsed}" "$(date --iso-8601=seconds)"
exit "${rc}"
